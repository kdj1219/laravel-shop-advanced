...
class EmailVerificationController extends Controller
{
    public function verify(Request $request)
    {
        ...
        if (!$email || !$token) {
            throw new InvalidRequestException('The link is incorrect.');
        }
        // 从缓存中读取数据，我们把从 url 中获取的 `token` 与缓存中的值做对比
        // 如果缓存不存在或者返回的值与 url 中的 `token` 不一致就抛出异常。
        if ($token != Cache::get('email_verification_'.$email)) {
            throw new InvalidRequestException('The link is incorrect or expired.');
        }
        ...
        // 最后告知用户邮箱验证成功。
        return view('pages.success', ['msg' => 'Email validation success.']);
    }
    ...
    public function send(Request $request)
    {
        ...
        if ($user->email_verified) {
            throw new InvalidRequestException('Your Email has already been verified.');
        }
